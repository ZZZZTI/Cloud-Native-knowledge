...
ifconfig / ip addr：查看IP、MAC、网卡状态（UP / DOWN）。
ethtool：查看网卡速率、双工模式、丢包。
route -n / ip route：查看内核路由表，理解数据包走哪个网卡。
套接字与连接：

netstat / ss：查看所有端口监听、连接状态、各状态连接计数。
lsof -i :端口：哪个进程占用了端口。
数据包捕获与分析：

tcpdump：命令行抓包。必须会常用参数（-i, -w, -r, host, port, tcp[tcpflags]）。
Wireshark：分析抓包文件。能看懂三次握手、重传、零窗口。
网络性能排查：

ping：测延迟和丢包（ICMP协议）。
traceroute / mtr：定位在哪一跳路由器上延迟突增或丢包。
nslookup / dig：排查DNS解析问题。
curl / wget：测试HTTP/HTTPS接口，查看详细请求响应头。
iperf3：测试带宽和吞吐量。

ip

ip a                      查看所有网卡及 IP 重点看：状态 UP/DOWN、inet 后的 IP/掩码、scope global（正常业务 IP）、scope host（如 lo 的 127.0.0.1） 识别网卡名：eth0, ens192, bond0 等
ip link set eth0 up/down  启用/关闭网卡
ip -s link show eth0      查看统计信息（丢包、错误）关注 RX 和 TX 的 errors, dropped, overruns。非零值通常表示物理层问题或队列满。
ip r                      查看路由表 重点关注 default via <网关IP>（默认路由） 直连网段（如 10.0.0.0/24 dev eth0 proto kernel）
ip neigh 或 arp -n        查看 ARP 缓存,显示 REACHABLE / STALE / FAILED。FAILED 可能表示邻居不可达。
